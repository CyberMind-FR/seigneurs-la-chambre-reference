...
- Rapport machine : `docs/migration-text-layer/PHASE_B_QR_BATCH.md` = PASS.
- `make validate` contrôle le référentiel historique, les QR bitmap sources et le lot QR SVG.
...
Les deux nouveaux registres opérationnels sont :
...
## Reconstitutions : règle désormais active
...
## Actifs actuellement déverrouillés
Selon `assets/ASSET_INDEX_V3.yaml` :

- `heraldry_la_chambre_svg` : `READY_RECONSTRUCTION`, rendu original à partir du blasonnement canonique de `pages/02.yaml` ;
- `page_10_castle_reconstruction_svg` : `READY_RECONSTRUCTION`, verrou géométrique sur `assets/reference/page-10-plan-reel.jpg` ;
- `ornament_bundle_svg` : `READY_ORNAMENT`, sans assertion historique ;
- les 16 QR SVG existants : `READY_FUNCTIONAL`.
...
1. produire `heraldry_la_chambre_svg` à partir du blasonnement canonique ;
2. produire `page_10_castle_reconstruction_svg` comme reconstitution non fac-similé ;
3. produire le bundle d’ornements sans texte ;
4. committer le logo association source validé pour débloquer sa vectorisation ;
5. ne produire le tracé documentaire direct du plan page 10 qu’après résolution des droits.

## Blocages permanents ou éditoriaux
...
- Page 10 : toute reconstitution doit respecter le plan réel comme autorité géométrique.
